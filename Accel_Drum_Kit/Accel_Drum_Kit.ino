...
//  dT = ( (double) accel_t_gyro.value.temperature + 12412.0) / 340.0;
  ...
  if(accel_t_gyro.value.z_accel-offset > threshold || accel_t_gyro.value.z_accel-offset < -1*threshold) {
    Serial.write(SNARE_DRUM);
  }
  if(!digitalRead(push) && millis() - last_push > depush) {
